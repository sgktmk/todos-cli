...
`j`/`k` で移動、`J`/`K` でセクション移動、`a` で追加、`s` でステータス変更、
`d` で完了、`/` で検索、`e` で外部エディタ、`?` でヘルプ、`q` で終了。

起動時はタスクがある最初のセクションを選ぶ。空のセクションでは `j`/`k` が
そのままセクション移動として働く。

複数行の本文編集は `$EDITOR` / `$VISUAL` に委譲する。
